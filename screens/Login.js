  ...
  return (
    <View style={{ marginBottom: 20 }}>
      <View className="flex-row justify-between items-center" style={{ marginBottom: 8 }}>
        <Text style={{ color: labelColor, fontSize: 10, fontWeight: '800', letterSpacing: 3, textTransform: 'uppercase' }}>
          {label}
        </Text>
        {error ? (
          <View className="flex-row items-center">
            <Ionicons name="alert-circle-outline" size={12} color="#f87171" style={{ marginRight: 3 }} />
            <Text style={{ color: '#f87171', fontSize: 11, fontWeight: '600', letterSpacing: 0.3 }}>{error}</Text>
          </View>
        ) : null}
      </View>
      ...
      <View
        ...
        style={{ height: 58, paddingHorizontal: 18, backgroundColor: bgColor, borderColor, borderWidth: 1.5 }}
      >
        <View
          className="items-center justify-center rounded-lg"
          style={{ width: 32, height: 32, backgroundColor: focused ? 'rgba(37,99,235,0.15)' : 'rgba(255,255,255,0.03)', marginRight: 12 }}
        >
          <Ionicons name={icon} size={16} color={iconColor} />
        </View>

        <TextInput
          style={{ flex: 1, color: '#e2e8f0', fontSize: 15.5, fontWeight: '400', letterSpacing: 0.2 }}
    ...
    >
      ...
      <View pointerEvents="none" className="absolute inset-0">
        {[0.15, 0.3, 0.45, 0.6, 0.75, 0.9].map((p, i) => (
          <View key={`h${i}`} style={{ position: 'absolute', top: height * p, left: 0, right: 0, height: 1, backgroundColor: 'rgba(30,64,175,0.06)' }} />
        ))}
        {[0.1, 0.3, 0.5, 0.7, 0.9].map((p, i) => (
          <View key={`v${i}`} style={{ position: 'absolute', left: width * p, top: 0, bottom: 0, width: 1, backgroundColor: 'rgba(30,64,175,0.06)' }} />
        ))}
        <Animated.View style={{ position: 'absolute', top: -200, left: -150, width: 480, height: 480, borderRadius: 240, backgroundColor: '#1e3a8a', opacity: glowOpacity }} />
        <Animated.View style={{ position: 'absolute', bottom: -160, right: -120, width: 380, height: 380, borderRadius: 190, backgroundColor: '#1d4ed8', opacity: glowOpacity }} />
        <View style={{ position: 'absolute', top: height * 0.38, right: -80, width: 220, height: 220, borderRadius: 110, backgroundColor: '#172554', opacity: 0.08 }} />
        {[
          { top: '6%',  left: '16%', s: 2.5, o: 0.55 }, { top: '11%', left: '84%', s: 2,   o: 0.4  },
          { top: '28%', left: '5%',  s: 1.5, o: 0.35 }, { top: '52%', left: '93%', s: 2,   o: 0.45 },
          { top: '70%', left: '10%', s: 2,   o: 0.5  }, { top: '85%', left: '72%', s: 1.5, o: 0.35 },
          { top: '20%', left: '58%', s: 1.5, o: 0.3  }, { top: '62%', left: '42%', s: 1,   o: 0.25 },
        ].map((d, i) => (
          <View key={`st${i}`} style={{ position: 'absolute', top: d.top, left: d.left, width: d.s, height: d.s, borderRadius: d.s, backgroundColor: `rgba(147,197,253,${d.o})` }} />
        ))}
      </View>
        ...
        >
          ...
          <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: titleSlide }] }} className="mb-11">
            <View style={{ marginBottom: 2 }}>
              <Text style={{ fontSize: 52, fontWeight: '900', letterSpacing: -2, lineHeight: 54, color: '#f1f5f9' }}>
                Welcome
              </Text>
              <Text style={{ fontSize: 52, fontWeight: '900', letterSpacing: -2, lineHeight: 58, color: '#2563eb' }}>
                Back
              </Text>
            </View>
            <View className="flex-row items-center" style={{ marginTop: 20, marginBottom: 14, gap: 10 }}>
            ...
            <Text style={{ color: '#475569', fontSize: 15, fontWeight: '400', lineHeight: 25, letterSpacing: 0.15 }}>
              Sign in to explore our curated{'\n'}product catalog.
            </Text>
          </Animated.View>

          {/* ── Form card ── */}
          <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: cardSlide }, { translateX: shakeAnim }] }}>
            <View
            ...
            >
              {/* Card accent bar */}
              <View style={{ height: 2, borderRadius: 1, backgroundColor: '#0f2044', marginBottom: 28, overflow: 'hidden' }}>
                <Animated.View style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '45%', borderRadius: 1, backgroundColor: accentAnim }} />
              </View>

              {/* Section label */}
              <Text style={{ color: '#1e3a5f', fontSize: 9, fontWeight: '800', letterSpacing: 3.5, textTransform: 'uppercase', marginBottom: 24 }}>
                Account credentials
              </Text>
              ...
              {apiError ? (
                <View style={{
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: '#ef4444', fontSize: 12, fontWeight: '700', letterSpacing: 0.3, marginBottom: 1 }}>
                      Login failed
                    </Text>
                    <Text style={{ color: '#7f1d1d', fontSize: 11, fontWeight: '500', lineHeight: 16 }}>
                      {apiError}
                    </Text>
                  </View>
              ...
              {strength && !passwordError && (
                <View style={{ marginTop: -8, marginBottom: 4 }}>
                  <View className="rounded-full overflow-hidden" style={{ height: 3, backgroundColor: '#0f172a' }}>
                    <View style={{ height: 3, width: strength.width, backgroundColor: strength.color, borderRadius: 99 }} />
                  </View>
                  <Text style={{ color: strength.color, fontSize: 10, fontWeight: '700', letterSpacing: 1.2, marginTop: 6, textTransform: 'uppercase' }}>
              ...
              {/* Forgot password */}
              <TouchableOpacity className="self-end" style={{ marginTop: 16, marginBottom: 28 }}>
                <Text style={{ color: '#3b82f6', fontSize: 13, fontWeight: '600', letterSpacing: 0.1, opacity: 0.75 }}>
                  Forgot password?
                </Text>
              </TouchableOpacity>
              ...
              >
                {loading ? (
                  <>
                    ...
                    <Text style={{ color: '#93c5fd', fontWeight: '700', fontSize: 12, marginLeft: 10, letterSpacing: 2.2, textTransform: 'uppercase' }}>
                      Signing in…
                    </Text>
                  </>
                ) : (
                  <>
                    <Text style={{ color: '#fff', fontWeight: '800', fontSize: 14, letterSpacing: 2, textTransform: 'uppercase', marginRight: 14 }}>
                      Sign In
                    </Text>
              ...
              >
                <Text style={{ color: '#1e3a5f', fontSize: 11, fontWeight: '600', letterSpacing: 0.3 }}>
                  Use demo account:{' '}
                  <Text style={{ color: '#2563eb', opacity: 0.7 }}>emilys / emilyspass</Text>
                </Text>
              </TouchableOpacity>

              {/* Divider */}
              <View className="flex-row items-center" style={{ marginTop: 24, marginBottom: 20 }}>
                <View className="flex-1" style={{ height: 1, backgroundColor: '#0f172a' }} />
                <Text style={{ color: '#1e3a5f', fontSize: 10, fontWeight: '600', letterSpacing: 2, textTransform: 'uppercase', marginHorizontal: 16 }}>
                  secured
                </Text>
                <View className="flex-1" style={{ height: 1, backgroundColor: '#0f172a' }} />
              </View>
              ...
              <View className="flex-row justify-center items-center" style={{ gap: 24 }}>
                {[
                  ...
                  <View key={b.label} className="items-center" style={{ gap: 5 }}>
                    <View className="items-center justify-center rounded-xl" style={{ width: 36, height: 36, backgroundColor: '#070f1f', borderWidth: 1, borderColor: '#0f2044' }}>
                      <Ionicons name={b.icon} size={15} color="#1e3a5f" />
                    </View>
                    <Text style={{ color: '#1e3a5f', fontSize: 9, fontWeight: '700', letterSpacing: 1.5, textTransform: 'uppercase' }}>
                      {b.label}
                    </Text>
                  </View>
                ))}
